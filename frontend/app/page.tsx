  ...
  const animationClasses = useMemo(
    () => ({
      header: prefersReducedMotion
        ? "opacity-100"
        : `transition-all duration-${isMobile ? "500" : "1000"} ${isLoaded ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"}`,

      hero: prefersReducedMotion
        ? "opacity-100"
        : `transition-all duration-${isMobile ? "500" : "1000"} ${isLoaded ? "scale-100 opacity-100" : "scale-95 opacity-0"}`,

      card: (index: number, isVisible: boolean) =>
        prefersReducedMotion
          ? "opacity-100"
          : `transition-all duration-${isMobile ? "300" : "500"} ${isVisible ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"}`,

      hover: prefersReducedMotion ? "" : isMobile ? "active:scale-95" : "hover:scale-105 hover:shadow-xl",
    }),
    ...
    <div className="min-h-screen bg-[#171221] text-white overflow-x-hidden">
      ...
      <header className={`border-b border-[#2e2447] px-6 py-4 ${animationClasses.header}`}>
        <div className="max-w-7xl mx-auto flex items-center justify-between">
              ...
              {["Novedades", "Hombres", "Mujeres", "Accesorios"].map((item, index) => (
                <a
                  ...
                  className={`text-[#a394c7] hover:text-white relative group ${
                    prefersReducedMotion
                      ? ""
                      : `transition-all duration-300 ${isLoaded ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"}`
                  }`}
                  style={{
          ...
          <div className={`flex items-center space-x-4 ${animationClasses.header}`}>
            <div className="relative group">
              <Search
                className={`absolute left-3 top-1/2 transform -translate-y-1/2 text-[#a394c7] w-4 h-4 ${prefersReducedMotion ? "" : "transition-colors duration-300 group-focus-within:text-[#7847eb]"}`}
              />
              <Input
                placeholder="Buscar"
                className={`pl-10 bg-[#2e2447] border-[#2e2447] text-white placeholder:text-[#a394c7] w-64 ${prefersReducedMotion ? "" : "transition-all duration-300 focus:border-[#7847eb] focus:ring-2 focus:ring-[#7847eb]/20"}`}
              />
            </div>
            <Button
              variant="outline"
      ...
      <section className="relative px-6 py-16" data-animate id="hero">
        <div className="max-w-7xl mx-auto">
          <div className={`relative rounded-2xl overflow-hidden ${animationClasses.hero}`}>
            <div
            ...
            >
              ...
              {!prefersReducedMotion && (
                <div className="absolute inset-0 opacity-30">
                  {(isMobile
                    ? [
                        { left: "33%", width: "3rem", height: "12rem", skew: "-skew-y-1" },
                        { left: "50%", width: "4rem", height: "16rem", skew: "skew-y-1" },
                        { right: "33%", width: "2.5rem", height: "10rem", skew: "-skew-y-2" },
                      ]
                    : [
                        { left: "25%", width: "2rem", height: "8rem", skew: "skew-y-2" },
                        { left: "33%", width: "3rem", height: "12rem", skew: "-skew-y-1" },
                        { left: "50%", width: "4rem", height: "16rem", skew: "skew-y-1" },
                        { right: "33%", width: "2.5rem", height: "10rem", skew: "-skew-y-2" },
                        { right: "25%", width: "3.5rem", height: "14rem", skew: "skew-y-1" },
                      ]
                  ).map((building, index) => (
                    <div
                      key={index}
                      ...
                      style={{
                        ...
                        willChange: "auto",
                        animation: isMobile ? "none" : `pulse 3s infinite ${index * 0.2}s`,
                      }}
                    />
                  ))}
                </div>
                  ...
                  {[...Array(particleCount)].map((_, i) => (
                    <div
                      ...
                      style={{
                        ...
                        top: `${Math.random() * 100}%`,
                        animation: `ping ${2 + Math.random() * 2}s infinite ${Math.random() * 3}s`,
              ...
              <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                <div className={`text-center z-10 ${animationClasses.hero}`}>
                  <h1 className={`text-5xl font-bold mb-4 ${prefersReducedMotion ? "" : "animate-pulse"}`}>
                    <span
                      className={`inline-block ${prefersReducedMotion ? "" : "hover:scale-110 transition-transform duration-300"}`}
                    >
                      Sube
                    </span>{" "}
                    <span
                    ...
                    <span
                      className={`inline-block text-[#7847eb] ${prefersReducedMotion ? "" : "hover:scale-110 transition-transform duration-300"}`}
                    >
                      Nivel
                    </span>{" "}
                    <span
                    ...
                    <span
                      className={`inline-block text-cyan-400 ${prefersReducedMotion ? "" : "hover:scale-110 transition-transform duration-300"}`}
                    >
                      Estilo!
                    </span>
                  </h1>
                  <p className={`text-xl text-[#e5e8eb] mb-8 ${animationClasses.hero}`}>
                    Armate y expresa tu estilo con ropa inspirada en la cultura Geek
                  </p>
                  <Button
      ...
      <section className="px-6 py-16" data-animate id="collections">
        <div className="max-w-7xl mx-auto">
          <h2 className={`text-3xl font-bold mb-8 ${animationClasses.hero}`}>Colecciones Populares</h2>
                ...
                <CardContent className="p-0">
                  <div
                    className={
                      ...
                      (collection.image ? " bg-black" : " bg-gradient-to-br " + collection.accent)
                    }
                  >
                    {collection.image && (
                      <img
                      ...
                      >
                        {index === 1 && (
                          <div className="w-full h-full flex items-center justify-center">
                            <div
                              className={`text-2xl font-bold text-gray-800 ${prefersReducedMotion ? "" : "animate-bounce"}`}
                            >
                              👕
                            </div>
                          </div>
                  ...
                  <div className="p-6">
                    <h3
                      className={`text-xl font-bold mb-2 ${prefersReducedMotion ? "" : "group-hover:text-[#7847eb] transition-colors duration-300"}`}
                    >
                      {collection.title}
                    </h3>
                    <p
                      className={`text-[#a394c7] text-sm ${prefersReducedMotion ? "" : "group-hover:text-white transition-colors duration-300"}`}
                    >
      ...
      <section className="px-6 py-16" data-animate id="arrivals">
        <div className="max-w-7xl mx-auto">
          <h2 className={`text-3xl font-bold mb-8 ${animationClasses.hero}`}>New Arrivals</h2>
          <div className="grid md:grid-cols-4 gap-6">
            {[
              ...
              <Card
                ...
                style={{
                  transitionDelay: prefersReducedMotion ? "0ms" : `${2 + index * 0.1}s`,
                  willChange: prefersReducedMotion ? "auto" : "transform, box-shadow",
                  contain: "layout style paint",
                }}
              >
                <CardContent className="p-0">
                  ...
                  <div className="p-4">
                    <h3
                      className={`font-bold mb-1 ${prefersReducedMotion ? "" : "group-hover:text-[#7847eb] transition-colors duration-300"}`}
                    >
                      {product.title}
                    </h3>
                    <p
      ...
      <section className="px-6 py-16 text-center" data-animate id="community">
        <div className={`max-w-4xl mx-auto ${animationClasses.hero}`}>
          ...
          <p className={`text-[#a394c7] text-lg mb-8 ${prefersReducedMotion ? "" : "animate-pulse"}`}>
      ...
      <footer className={`border-t border-[#2e2447] px-6 py-8 ${animationClasses.hero}`}>
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-wrap justify-between items-center mb-8">
            <div className="flex flex-wrap space-x-8">
              {["Acerca de Nosotros", "Contacto", "FAQ", "Políticas de Privacidad", "Términos de Servicio"].map(
                (link, index) => (
                  <a
                    key={link}
                    href="#"
          ...
          <div className={`text-center text-[#a394c7] text-sm ${prefersReducedMotion ? "" : "animate-pulse"}`}>
      ...
      {!isLoaded && (
        <div className="fixed inset-0 bg-[#171221] z-50 flex items-center justify-center">
          <div className="text-center">
            ...
            ></div>
            <p className={`text-[#a394c7] ${prefersReducedMotion ? "" : "animate-pulse"}`}>Cargando Asgard Store...</p>
          </div>
        </div>
      )}
    </div>
